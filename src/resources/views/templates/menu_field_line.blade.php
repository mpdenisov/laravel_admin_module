    ...
    <td>
        ...
        <!-- File size limit -->
        <label class="size">{{ trans('Admin::templates.templates-menu_field_line-size_limit') }}</label>
        <input type="text" name="f_size[]" value="{{ old('f_size.'.$index, '2') }}" class="form-control size"
               placeholder="{{ trans('Admin::templates.templates-menu_field_line-size_limit_placeholder') }}" style="display: none;">
        <!-- /File size limit -->

        <!-- File dimensions limit -->
        <label class="dimensions">{{ trans('Admin::templates.templates-menu_field_line-maximum_width') }}</label>
        <input type="text" name="f_dimension_w[]" value="{{ old('f_dimension_w.'.$index, '4096') }}"
               class="form-control dimensions"
               placeholder="{{ trans('Admin::templates.templates-menu_field_line-maximum_width_placeholder') }}" style="display: none;">
        <label class="dimensions">{{ trans('Admin::templates.templates-menu_field_line-maximum_height') }}</label>
        ...
        <select name="f_texteditor[]" class="form-control texteditor" style="display: none;">
            <option value="0"
                    @if($key == old('f_texteditor.'.$index)) selected @endif>{{ trans('Admin::templates.templates-menu_field_line-dont_use_ckeditor') }}
            </option>
            <option value="1"
                    @if($key == old('f_texteditor.'.$index)) selected @endif>{{ trans('Admin::templates.templates-menu_field_line-use_ckeditor') }}
            </option>
        </select>
        <!-- /Use ckeditor on textarea field -->
        ...
        <select name="f_relationship[]" class="form-control relationship" style="display: none;">
            <option value="">{{ trans('Admin::templates.templates-menu_field_line-select_relationship') }}</option>
        ...
        <!-- ENUM values -->
        <label class="enum">{{ trans('Admin::templates.templates-menu_field_line-enum_values') }}</label>
        <input type="text" name="f_enum[]" value="{{ old('f_enum.'.$index) }}" class="form-control enum tags"
               placeholder="{{ trans('Admin::templates.templates-menu_field_line-enum_values_placeholder') }}" style="display: none;">
        <!-- /ENUM values -->
    </td>
    <td>
