...
class ModelBuilder
{
    ...
    public function build()
    {
        ...
        $this->name      = $cached['singular_name'];
        $this->tableName = $cached['plural_name'];
    ...
    */
    ...
    {
        file_put_contents(app_path('Models' . DIRECTORY_SEPARATOR . $this->fileName), $template);
    }
    ...
    {
        if($input != \'0000-00-00\') {
            return Carbon::createFromFormat(\'Y-m-d\', $input)->format(config(\'admin.date_format\'));
        }else{
            return \'\';
        }
    }' . "\r\n\r\n";
